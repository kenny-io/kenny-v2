...
const handleOnNewsletterSubscribe = async () => {
    try {
        ...
        // Subsribe
        const subscriptionResponse = await useMailchimp(state.subscriberEmail, state.subscriberName);
                    ...
                    <div>
                        <ul class="flex flex-rows text-[#6F6F6F] gap-2">
                            <li v-for="link in socialLinks" :key="link.name">
                                <a :href="link.url" target="_blank" rel="noopener noreferrer" class="uppercase">
                                    {{ link.name }}
                                </a>
                            </li>
                        </ul>
